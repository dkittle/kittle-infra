...
fun run(ctx: Context) {
    runBlocking {
        ...
        val privateSGName = privateSubnetGroup.name.applyValue(fun(name: String): String { return name })
        ...
        ctx.export("privateSubnetGroupName", privateSGName)
        ctx.export("vpcId", vpcId)
        ctx.export("igwId", igwId)
        ctx.export("routeTableId", routeTableId)
